...
and highly automatic templates. FormHandler is designed to
make it easy to produce custom rendering modules.
...
and use them to perform field validation. You can use validation routines
from an L<Exporter> based class.
...
An example of a form class;
    ...
    has_field 'age'     => ( type => 'PosInteger', required => 1, wrapper_class => 'box',
    ...
    sub hobby_options { ( 1 => 'Stamps', 2 => 'Cars' ) }
...
You might also want to pass in the 'action':
...
If the validation succeeds, and the form is not a database form,
you can get a struct of validated values from C<< $form->value >> for
you to handle.

The values to be used to fill in your form are automatically created
by FormHandler, and are available in the field's 'fif' attribute, with
C<< $field->fif >>, or in the form's fif hash, which will contain the
fill-in-form values for all the form's fields (C<< $form->fif >>).
...
=head2 Database forms

If you are using the DBIC model, you extend L<HTML::FormHandler::Model::DBIC>
instead of 'HTML::FormHandler'. You need to provide either an item - a DBIC
row - or the item_id, item_class (result source name) and schema.

When the 'process' method is executed, the database will be updated if the
validation succeeds.

Select options can be automatically retrieved from the database for single and
multiple selects. See L<HTML::FormHandler::Model::DBIC> and the select field
...
The 'value' structure is the same structure that FormHandler uses to update the database.
...
C< use namespace::autoclean; > in the end of the package to keep the
namespace clean of imported methods.)
...
(If your 'address' is a relationship in the 'Person' result source instead of
just another set of 'Person columns, that you probably want to use a compound
field, 'address'.)
...
An array of name / definition pairs to define fields.
...
Note: do not set the 'value' attribute yourself in an attempt
to provide a field default. It won't work, since it will be cleared out
when 'process' is run. Use one of the other many methods to set defaults.
It is acceptable to set the value in a validation method, however, as
a 'transform'.
...
which you don't want to do until the form has been submitted.

=head1 Other methods for your form

=over 4

=item options_$fieldname, options_method
...
provide them from an options method.

An options method can either be set using the field attribute, or you can
create a form method '<option_$fieldname', which will automatically be used.

   has_field 'foo' => ( type => 'Select', options_method => \&my_options );

Note that a method set with 'options_method' is a method on the field, and
a method set by default with 'options_$fieldname' is a method on the form.

An options method should return a list of ordered key (option
...
See L<HTML::FormHandler::Field::Select> for more information.

=item default_$fieldname, default_method

Allows you to provide (in the form) a different initial value for a particular
field than that in the database.

   has_field 'foo' => ( default_method => \&my_foo );
   sub my_foo {
      my $self = shift; # $self is the 'foo' field
      return '...';
   }

   has_field 'license';
   sub options_default {
      my ( $self, $field, $item ) = @_; # $self is the form
...
=item validate_$fieldname, validate_method
...
HTML form if you are not using FormHandler rendering.
